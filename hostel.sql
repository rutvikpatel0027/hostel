...
CREATE TABLE `student_attendance` (
  ...
  `status` enum('present','absent','on_leave','not_marked') DEFAULT 'not_marked',
...
CREATE TABLE `entry_exit_logs` (
  ...
  `entry_point` varchar(100) DEFAULT 'Main Gate',
  `exit_point` varchar(100) DEFAULT 'Main Gate',
  `status` enum('in','out','away') DEFAULT 'in',
  `created_at` timestamp DEFAULT current_timestamp(),
  `updated_at` timestamp DEFAULT current_timestamp() ON UPDATE current_timestamp()
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE `attendance_alerts` (
  `id` int(11) NOT NULL,
  `registration_id` int(11) NOT NULL,
  `alert_type` enum('late_entry','no_exit','low_attendance','absent') DEFAULT 'absent',
...
ALTER TABLE `student_attendance`
  ...
  ADD UNIQUE KEY `unique_attendance` (`registration_id`, `attendance_date`);
